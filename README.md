...
当前仓库正在进行 **PR 13**：禁用运行时默认 1kHz 测试音源。PR7 / `v1.0.0-demo` 为最终测试报告与 Release，PR8 新增指标演示辅助页面，PR9 曾用 App 内 1kHz 测试音验证 AAC 128Kbps 封装链路，PR10 补充 ffprobe、动态码率和延迟证据，PR12 修复 HTTP 流握手空响应问题。本项目已完成一个 Android DLNA 投屏 Demo 的可演示链路：小米 14 采集屏幕并编码为 H.264，通过本地 HTTP `live.ts` 输出，再由 App 使用 DLNA AVTransport 控制 Kodi 播放该流。PR13 保留 AAC / ADTS / MPEG-TS 音频封装能力，但默认流恢复为 H.264 video-only；真实系统播放音频待 PR14 实现和真机验证。
...
| 音频规格 | `AAC 128 Kbps` | ffprobe 音频流检查 + 真机听感验证 | PR9 曾用 App 内置 1kHz 测试音验证封装链路；PR13 起默认禁用运行时测试音，当前默认 H.264 video-only。真实系统播放音频待 PR14 实现和验证。 | 待 PR14 |
...
目标值不代表已达成结果。PR 5 已在当前真机和 PC 热点环境验证 H.264 MPEG-TS 本地流可抓取、可识别并可实时解码；PR 6 增加 AVTransport 控制命令发送和错误映射，并在 Kodi 上完成最小链路演示。PR 7 新增 ADB forward + `curl` + `ffprobe` 证据，证明 App 本机 HTTP 服务和 `/live.ts` 内容可读；该证据不等同于 Windows 局域网直连成功，也不等同于真实电视端可访问。PR10 曾复测到 `Empty reply from server`，PR12 已修复 HTTP 握手路径；PR13 只禁用默认测试音，不能把 AAC、动态码率或延迟写成已达成。
...
音频
目标：AAC 128Kbps
当前：PR13 默认 video-only；运行时默认测试音已禁用
证据：PR9 曾用 1kHz 测试音验证封装链路；真实系统播放音待 PR14 实现和验证
```
...
| PR 13 | 禁用运行时默认测试音源，默认 video-only | 当前 PR |
| PR 14 | AudioPlaybackCapture 系统播放音采集 | 待实现 / 验证 |

## 已知问题

- 部分路由器、防火墙或 Renderer 实现可能影响 SSDP 发现。
- Android 14+ 的整屏 / 单应用选择弹窗样式和单应用选择流程可能受厂商 ROM 影响。
- PR13 起默认 MPEG-TS 只声明 H.264 video-only；PR9 的测试音封装能力保留，但不在运行时默认启用。
- PR10 曾发现 `/live.ts` 连接后返回 `Empty reply from server`；PR12 已修复 HTTP 握手路径。若后续复测失败，应保存新证据而不是沿用历史结论。
- 真实系统播放音频待 PR14 实现和真机验证；`ffprobe audio=aac` 只能证明存在 AAC 音轨，不能单独证明真实播放音链路跑通。
