...
当前仓库正在进行 **PR 10** 的最终指标证据补强。PR7 / `v1.0.0-demo` 为最终测试报告与 Release，PR8 新增指标演示辅助页面，PR9 新增 AAC 128Kbps App 内测试音轨封装，PR10 复测 ffprobe、动态码率和延迟证据。本项目已完成一个 Android DLNA 投屏 Demo 的可演示链路：小米 14 采集屏幕并编码为 H.264，通过本地 HTTP `live.ts` 输出，再由 App 使用 DLNA AVTransport 控制 Kodi 播放该流。PR9 同时封装了 AAC 音频测试轨道（1kHz 正弦波），但 PR10 当前发现 `/live.ts` 返回空响应，尚未取得可用于 ffprobe 的音视频样本；测试音不代表系统内录或麦克风采集已实现。
...
| 音频规格 | `AAC 128 Kbps` | ffprobe 音频流检查 | PR9 新增 App 内置 1kHz 测试音轨，目标编码参数 AAC-LC、128Kbps、48kHz；PR10 未取得 `.ts` 样本，ffprobe 尚未验证 `audio=aac`。未实现：系统内录、麦克风采集。 | 测试音轨已接入，ffprobe 未验证 |
...
目标值不代表已达成结果。PR 5 已在当前真机和 PC 热点环境验证 H.264 MPEG-TS 本地流可抓取、可识别并可实时解码；PR 6 增加 AVTransport 控制命令发送和错误映射，并在 Kodi 上完成最小链路演示。PR 7 新增 ADB forward + `curl` + `ffprobe` 证据，证明 App 本机 HTTP 服务和 `/live.ts` 内容可读；该证据不等同于 Windows 局域网直连成功，也不等同于真实电视端可访问。PR10 当前复测发现手机端 `*:8080` 监听存在，但 PC 直连和手机本机 curl 均返回 `Empty reply from server`，因此不能把 AAC、动态码率或延迟写成已达成。
...
- App 可安装启动，首页已更新为“PR 10：最终指标证据补强”。
- 采集可启动，UI 显示 `c2.qti.avc.encoder`、`1080 x 1920`、`8.0 Mbps`、`CBR`。
- UI 显示本地流地址 `http://192.168.137.138:8080/live.ts`，格式为 `MPEG-TS + H.264 视频 + AAC 测试音轨`。
...
PR10 不修复 `LocalStreamServer` / `StreamSession` 核心链路。后续建议另开 `fix/stream-session-empty-reply`，先排查 HTTP 会话为何静默关闭，再复测 AAC、动态码率和延迟。
...
音频
目标：AAC 128Kbps
当前：AAC 128Kbps 测试音轨已接入
证据：需可读取的 live.ts 样本；PR10 当前 /live.ts 返回 Empty reply，ffprobe 未取得有效样本
```
...
- PR10 当前发现 `/live.ts` 连接后返回 `Empty reply from server`，导致无法抓取 `.ts` 样本并用 ffprobe 复核 AAC / 动态码率；需后续小 PR 排查 StreamServer 会话。
- MPEG-TS 音频尾包当前使用 payload-only padding；ffprobe / Kodi 兼容性尚未通过 PR10 样本验证，严格 DMR 设备兼容性待测。
- `AudioEncoder` 启动失败路径存在潜在线程释放风险，建议后续另开 `fix/audio-encoder-cleanup`。
