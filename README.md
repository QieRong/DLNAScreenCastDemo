...
当前代码已进入 **PR 17**：Kodi 慢动作播放与秒表严重延迟积压的分层诊断。PR7 / `v1.0.0-demo` 为最终测试报告与 Release，PR8 新增指标演示辅助页面，PR9 曾用 App 内 1kHz 测试音验证 AAC 128Kbps 封装链路，PR10 补充 ffprobe、动态码率和延迟证据，PR12 修复 HTTP 流握手空响应问题，PR13 禁用运行时默认测试音，PR14 接入 AudioPlaybackCapture 真实系统播放音代码路径，PR16A / PR16B 补充 DLNA metadata/header 与全局时间基 / per-track PTS 单调修正。PR17 只新增慢动作和严重延迟的诊断日志、video-only / video+audio 对照记录模板与最小时间域保护；当前不能写任何低延迟达标或 Kodi 慢动作已完成修复的结论。PR17 记录见：[docs/evidence/pr17-kodi-slow-motion-diagnostics.md](docs/evidence/pr17-kodi-slow-motion-diagnostics.md)。
...
| 视频码率 | `8 Mbps` | MediaCodec 配置 + 样本估算 | 目标配置 `8 Mbps`；PR10 当前 `/live.ts` 空响应，未取得 30 秒动态样本 | 待复测 |
| 音频规格 | `AAC 128 Kbps` | ffprobe 音频流检查 + logcat PCM peak/RMS + 真机听感验证 | PR14 已接入 AudioPlaybackCapture 到 AAC / MPEG-TS 音频 PID 的代码路径；归档样本 `pr14-audio-live-aac.ts` 可识别 `aac`、约 `130 Kbps`，解码 WAV 非静音；但接收端听到目标媒体声音仍未归档。 | 样本层通过 / 接收端待验证 |
...
目标值不代表已达成结果。PR 5 已在当前真机和 PC 热点环境验证 H.264 MPEG-TS 本地流可抓取、可识别并可实时解码；PR 6 增加 AVTransport 控制命令发送和错误映射，并在 Kodi 上完成最小链路演示。PR 7 新增 ADB forward + `curl` + `ffprobe` 证据，证明 App 本机 HTTP 服务和 `/live.ts` 内容可读；该证据不等同于 Windows 局域网直连成功，也不等同于真实电视端可访问。PR10 曾复测到 `Empty reply from server`，PR12 已修复 HTTP 握手路径；PR13 只禁用默认测试音。PR14 不能把 AAC 音轨、抖音声音、手机端静音或真实播放音端到端写成已达成，除非对应真机证据已经补齐。PR14 当前证据详见：[docs/evidence/pr14-playback-audio-capture-evidence.md](docs/evidence/pr14-playback-audio-capture-evidence.md)。
...
flowchart LR
    ...
    SERVICE --> AUDIO_CAPTURE["AudioPlaybackCapture + AudioRecord"]
    AUDIO_CAPTURE --> AUDIO_ENCODER["AAC-LC 128Kbps encoder"]
...
- 新增 DLNA 播放控制卡，提供独立按钮：
  - “发送到 Renderer / 开始播放”：PR14 起会先执行一次 best-effort `Stop` 清理 Renderer 旧播放状态，再使用带 timestamp nonce 的 `/live.ts` 执行 `SetAVTransportURI` + `Play`。
...
**当时目标**：在现有 H.264 MPEG-TS 基础上增加 AAC 音频测试轨道，使 ffprobe 能识别 `audio=aac`。

**当前状态（PR13 后）**：PR9 的 AAC 编码、ADTS 封装和 MPEG-TS 音频封装能力保留，但运行时默认不再启动 App 内测试音源。默认流为 H.264 video-only；这不代表系统声音、抖音声音投屏或远端独占输出已经实现。
...
- **未申请**：`RECORD_AUDIO` 权限。
- 测试音仅用于当时验证 AAC 封装链路，不代表真实屏幕声音已采集；PR13 起默认禁用运行时测试音。
...
- 修改 `capture/ScreenCaptureManager.kt`：PR9 时 `start()` 会创建并启动 `AudioEncoder`；PR13 起已默认停用该运行时测试音路径。
...
**本阶段不实现**：系统内录、麦克风采集、AAC 与视频真实内容同步、Kodi 音频播放兼容性验证。PR13 后默认运行时不再输出测试音。

## PR 10 最终指标证据补强

PR10 的目标是补充真机验收证据，而不是新增核心功能。证据文件见：[docs/evidence/pr10-http-stream-evidence.md](docs/evidence/pr10-http-stream-evidence.md)。

当前复测结果：

- PR10 当时 App 可安装启动，首页已更新为“PR 10：最终指标证据补强”。
- 采集可启动，UI 显示 `c2.qti.avc.encoder`、`1080 x 1920`、`8.0 Mbps`、`CBR`。
- PR10 当时 UI 显示本地流地址 `http://192.168.137.138:8080/live.ts`，格式为 `MPEG-TS + H.264 视频 + AAC 测试音轨`；PR13 后默认禁用该测试音路径。
- 手机端 `ss -ltnp` 显示 `*:8080` 正在监听。
- PC 局域网直连 `http://192.168.137.138:8080/live.ts` 能建立 TCP 连接，但返回 `Empty reply from server`。
- 手机本机访问 `http://127.0.0.1:8080/live.ts` 同样返回 `Empty reply from server`。
- 因未取得 `.ts` 样本，本轮无法执行有效 `ffprobe` 音视频流识别；不能写 `audio=aac` 已通过。
- 30 秒动态码率样本和延迟三次读数依赖可读取的 `/live.ts`，本轮保持未实测。

PR10 不修复 `LocalStreamServer` / `StreamSession` 核心链路。该握手路径已在 PR12 修复；后续复测必须保存新证据，不能沿用历史空响应结论。

## PR 13 禁用运行时默认测试音源

PR13 的目标是避免把 App 内测试音误解为真实系统声音投屏。本 PR 不删除 AAC 能力，只禁用默认运行时假音源。

- 保留：AAC 编码参数、ADTS 头封装、`muxAudioAccessUnit()`、MPEG-TS 音频封装单元测试。
- 默认：`ScreenCaptureManager.start()` 不再启动 App 内测试音编码器，`MpegTsMuxer` 默认 `includeAudio=false`，PMT 不声明 audio PID。
- 当前结论：默认流为 MPEG-TS + H.264 video-only；系统声音、抖音声音投屏和远端独占输出未实现。
- PR14：真实系统播放音计划单独放到 `feat/014-playback-audio-capture`，使用 `MediaProjection + AudioPlaybackCaptureConfiguration + AudioRecord + RECORD_AUDIO`，并按真机结果记录目标 App 是否允许捕获。

## PR 14 AudioPlaybackCapture 真实系统播放音采集

PR14 的目标是把真实系统播放音接入现有 `/live.ts`：`AudioPlaybackCapture + AudioRecord` 读取 PCM，`AudioEncoder` 编码为 AAC-LC 128Kbps，再由 `MpegTsMuxer` 写入音频 PID。本 PR 不做麦克风录音，不恢复 App 内 1kHz 测试音，也不提前宣称抖音声音或真实系统音频已达成。音画同步只做“基本音画同步 / 人工观察级同步”验证，不承诺严格毫秒级 AV sync。

- Manifest 本阶段只新增 `RECORD_AUDIO`；不新增 `FOREGROUND_SERVICE_MICROPHONE`，`ScreenCaptureService` 仍只声明 `mediaProjection` foreground service type。
- 权限流程：开始采集前先请求 `RECORD_AUDIO`；拒绝时继续允许 MediaProjection 录屏并降级为 video-only；本次会话不反复弹录音权限。
- 捕获限制：AudioPlaybackCapture 只能捕获 Android 10+、同一用户资料下、usage 为 `MEDIA` / `GAME` / `UNKNOWN` 且目标 App capture policy 允许的播放音。目标 App 可能禁止捕获。
- 合并门槛：第一轮先用本地 MP4 验证链路，优先选择画面有倒计时、击掌或节奏点且声音可辨认的视频；抖音只作为附加测试项。如果抖音采不到，只能记录“目标 App 可能限制音频捕获”，不能写成“系统音频采集失败”。
- 声音路由边界：PR14 只承诺捕获真实播放音并让接收端播放；不承诺手机端静音或远端独占出声。手机是否仍外放取决于系统音频路由和设备行为，后续如需要再单独研究。
- UI 音频状态按阶段展示：权限未授权、API 不支持、AudioRecord 已启动、first PCM frame、PCM peak/RMS 非静音、first AAC frame、first audio TS packet、可能静音 / 目标 App 不允许捕获、已降级 video-only。
- PR14 验收结论分四层：`ffprobe audio=aac` 只证明 AAC 音轨存在；logcat 有 PCM peak/RMS + first AAC frame 才能写已捕获到有效播放音并完成编码；PC/Kodi/ffplay 能听到目标媒体声音才能写接收端真实音频播放链路跑通；vivo X80 录像观察声音和画面节奏点基本一致，才能写“人工观察级基本音画同步”。
- 当前已归档样本：`docs/evidence/artifacts/pr14-audio-live-aac.ts` 的 `ffprobe` 结果包含 `h264` + `aac`，音频 `48000 Hz`、单声道、约 `130 Kbps`；`docs/evidence/artifacts/pr14-audio-live-decoded.wav` 的 `astats` 结果为 `RMS level dB: -21.376643`，不是全零静音。该样本仍不能替代接收端听感证据。
...
- Android 10+ PR14 首次开始采集时先请求 `RECORD_AUDIO` 用于 AudioPlaybackCapture 系统播放音采集；拒绝时继续录屏并降级为 video-only。
...
2. 打开 Kodi，并确认 UPnP / DLNA 与远程控制已开启。
3. 第一轮先打开本地 MP4，优先使用有倒计时、击掌或节奏点且声音清楚的视频；确认手机媒体音量不为 0。
4. 启动 App，授权 `RECORD_AUDIO` 和系统录屏权限。
5. App 点击“搜索 DLNA 设备”，确认列表显示 Kodi。
6. App 点击“开始采集”，确认首页显示 `/live.ts`。
7. 确认没有 `ffplay` / `curl` 占用 `/live.ts`。
8. 在设备卡点击“选择 Renderer”。
9. 点击“发送到 Renderer / 开始播放”。PR14 会先 best-effort `Stop`，再发送带 nonce 的 `/live.ts`。
10. 记录 App 状态、`adb logcat -s DlnaControl StreamServer ScreenCapture Encoder`、Kodi 画面表现，并用 vivo X80 录制接收端画面和声音用于人工观察级同步判断。

验收结果必须区分三类：

1. best-effort `Stop` 后 `SetAVTransportURI` + `Play` 成功，Kodi 播放出手机画面。
...
音频
目标：AAC 128Kbps
当前：PR14 已接入真实系统播放音采集代码路径；归档样本可识别 AAC 且解码 WAV 非静音
证据：`docs/evidence/pr14-playback-audio-capture-evidence.md`；接收端听感和手机端自动静音仍待补证
```
...
| PR 9 | AAC 128Kbps 测试音轨封装（App 内生成 1kHz 测试音） | 已合并 |
| PR 10 | 最终指标证据补强与文档复测 | 已合并 |
| PR 13 | 禁用运行时默认测试音源，默认 video-only | 已合并 |
| PR 14 | AudioPlaybackCapture 系统播放音采集 | 已合并 / 样本层通过，接收端听感待验证 |
| PR 16A | DLNA streaming headers + DIDL-Lite metadata | 已合并 |
| PR 16B | MPEG-TS pipeline 全局 baseTime / per-track PTS 单调修正 | 已合并 |
| PR 17 | Kodi 慢动作与严重延迟积压分层诊断 | 当前 PR / 待真机对照 |

## 已知问题

- 部分路由器、防火墙或 Renderer 实现可能影响 SSDP 发现。
- Android 14+ 的整屏 / 单应用选择弹窗样式和单应用选择流程可能受厂商 ROM 影响。
- PR13 起默认 MPEG-TS 不再使用 App 内 1kHz 测试音；PR14 改为真实播放音捕获路径。
- PR10 曾发现 `/live.ts` 连接后返回 `Empty reply from server`；PR12 已修复 HTTP 握手路径。若后续复测失败，应保存新证据而不是沿用历史结论。
- AudioPlaybackCapture 只能捕获 Android 10+、同一用户资料下、usage 为 MEDIA/GAME/UNKNOWN 且目标 App capture policy 允许的播放音；目标 App 可能禁止捕获。
- PR14 已归档一份 `ffprobe` 可识别 AAC 且解码 WAV 非静音的样本；这只能证明样本层音轨和非静音音频存在，不能替代 PC/Kodi/ffplay 听到目标 App 声音的证据。
- 投屏软件期望接收端出声、手机端不双重外放；PR14 当前未证明手机端自动静音策略，不能写成已达成。
...
- Kodi 播放 `/live.ts` 存在周期性缓冲 / 卡顿；PR17 正在新增慢动作和严重延迟积压诊断，实际改善必须以 video-only / video+audio、Kodi / ffplay、curl + ffprobe 对照结果为准。
...
默认不提交抓包、外部摄像机原始视频或大体积视频；PR14 仅归档体积可控的 `.ts` / `.wav` 样本用于证明 AAC 音轨和非静音音频能量。当前小米 ROM 在采集期间执行 `adb screencap` 可能触发系统停止采集，因此截图优先使用外部拍摄或 PC 侧可公开画面。
...
已知问题：Kodi 周期性缓冲、Windows 直连历史上曾超时、PR10 live.ts 当前空响应、严格延迟未实测、真实电视兼容未覆盖
未实现项：AAC 128 Kbps、HLS、DIDL-Lite metadata / DLNA contentFeatures 兼容增强、多电视兼容矩阵
```

说明：以上 Release 信息对应已发布的 `v1.0.0-demo` 历史 APK。当前 PR14 工作区已新增 AudioPlaybackCapture 代码路径和样本层 AAC 证据，但尚未发布为新的 Release APK。
