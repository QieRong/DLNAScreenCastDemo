...
当前代码已进入 **PR 14**：AudioPlaybackCapture 真实系统播放音采集收口。PR7 / `v1.0.0-demo` 为最终测试报告与 Release，PR8 新增指标演示辅助页面，PR9 曾用 App 内 1kHz 测试音验证 AAC 128Kbps 封装链路，PR10 补充 ffprobe、动态码率和延迟证据，PR12 修复 HTTP 流握手空响应问题，PR13 禁用运行时默认测试音。本项目已完成一个 Android DLNA 投屏 Demo 的可演示链路：小米 14 采集屏幕并编码为 H.264，通过本地 HTTP `live.ts` 输出，再由 App 使用 DLNA AVTransport 控制 Kodi 播放该流。PR14 已接入 `MediaProjection + AudioPlaybackCaptureConfiguration + AudioRecord -> AAC -> MPEG-TS` 代码路径，并归档一份 `ffprobe` 可识别 H.264 + AAC、解码 WAV 非静音的样本；PC/Kodi/ffplay 接收端听感、目标 App capture policy、手机端自动静音仍必须按后续真机证据记录。
...
| PR 14 | AudioPlaybackCapture 系统播放音采集 | 当前 PR / 样本层通过，接收端听感待验证 |
