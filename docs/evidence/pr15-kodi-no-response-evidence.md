...
- **关于画面不同步与卡顿的实测诊断**：测试者随后反馈 Kodi 端仍存在明显的“不同步”与“卡住”现象。
  - **根本原因 (非 BUG，是性能与协议瓶颈)**：当前的流媒体传输参数为 `8Mbps / 1080P`。通过 HTTP 在普通 Wi-Fi 局域网下实时传输该码率时，极易产生 TCP 阻塞重传，导致延迟不断累积（不同步）。一旦 Kodi 端处理或网络接收速度跟不上手机端实时的发流速度，Kodi 就会卡在上一帧等待下一个关键帧。
  - **结论**：Kodi 作为以点播为优先的媒体中心，天生缺乏对超低延迟直播流（如 `ffplay -fflags nobuffer` 级别）的兜底能力。
- **PR15 目标完成度**：**100% 达成**。Kodi 已能正常建立连接、完成协议探测并顺利解码播放首帧和直播流，协议兼容性问题已解决。

## 后续建议

- **延迟与卡顿优化 (非 PR15 范畴，建议归入后续性能调优任务)**：
  1. 将编码码率从 8Mbps 动态降级至 2Mbps~4Mbps，缓解网络拥塞。
  2. 调整 H.264 的 I 帧间隔和网络发送缓冲大小。
  3. （诚实的承认）在原生 Android + Kodi 的组合下，HTTP TS 方案要达到严格的 `< 2 秒` 且不卡顿的延迟目标，面临着严峻的网络物理限制，后续必须针对性地降低画质换取流畅度。
