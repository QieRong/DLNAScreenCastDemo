...
```text
当前阶段：PR 10：最终指标证据补强
性能指标：PR10 当时补强 ffprobe、动态码率和延迟证据；当时 UI 显示 AAC 测试音轨边界，是否被识别以真机样本为准；延迟仍需外部录像实测。PR13 后运行时默认禁用该测试音路径。
...
格式：PR10 当时 UI 显示 MPEG-TS + H.264 视频 + AAC 测试音轨；PR13 后默认 video-only
```
