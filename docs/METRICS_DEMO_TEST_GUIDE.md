...
> PR8 只新增指标演示辅助页面和测试文档，不修改 PR7 / `v1.0.0-demo` 的历史测试结论，不移动已发布 tag，不覆盖已发布 APK 说明。PR8 的目标是提供“可复现测试入口”，不是宣称指标达成。PR9 曾使用 App 内测试音验证 AAC 封装链路；PR13 起默认禁用运行时测试音。PR14 接入 AudioPlaybackCapture 真实系统播放音代码路径，但真机结论必须按 ffprobe、logcat 和听感证据分层记录；若 `/live.ts` 不能读取，所有相关指标都必须继续写未实测或未验证。
...
PR14 后样本可能是 video-only，也可能包含真实播放音 AAC 音轨，取决于 `RECORD_AUDIO` 授权、Android 版本、目标 App usage 和 capture policy。AudioPlaybackCapture 只能捕获 Android 10+、同一用户资料下、usage 为 MEDIA/GAME/UNKNOWN 且目标 App 允许捕获的播放音；目标 App 可能禁止捕获。

当前已归档一份 PR14 样本：`docs/evidence/artifacts/pr14-audio-live-aac.ts` 可由 `ffprobe` 识别出 `h264` + `aac`，音频 `48000 Hz`、单声道、约 `130 Kbps`；`docs/evidence/artifacts/pr14-audio-live-decoded.wav` 经 `astats` 检查为非全零静音。完整记录见 `docs/evidence/pr14-playback-audio-capture-evidence.md`。

PR14 证据必须分层记录：

```text
ffprobe 出现 audio=aac：只证明有 AAC 音轨。
解码 WAV 非静音：证明归档样本中存在非静音音频能量。
logcat 出现 PCM peak / RMS + first AAC frame：证明读到非静音 PCM 并完成 AAC 编码。
PC / Kodi / ffplay 能听到视频 App 声音：才说明真实播放音频链路跑通。
```

不允许只凭 `audio=aac` 或一份非静音 WAV 写“真实系统音频已达成”。先用允许捕获的媒体源验证链路，抖音只作为附加测试项；如果抖音采不到，只能记录“目标 App 可能限制音频捕获”，不能写成“系统音频采集失败”。投屏体验期望接收端出声、手机端不双重外放；手机端自动静音策略未实测前，继续写“未证明”。
