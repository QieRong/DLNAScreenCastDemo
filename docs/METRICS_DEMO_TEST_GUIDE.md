...
> PR8 只新增指标演示辅助页面和测试文档，不修改 PR7 / `v1.0.0-demo` 的历史测试结论，不移动已发布 tag，不覆盖已发布 APK 说明。PR8 的目标是提供“可复现测试入口”，不是宣称指标达成。PR9 曾使用 App 内测试音验证 AAC 封装链路；PR13 起默认禁用运行时测试音。PR14 真实系统播放音采集仍待实现和真机验证；若 `/live.ts` 不能读取，所有相关指标都必须继续写未实测或未验证。
...
PR13 后默认运行时测试音已禁用，因此默认样本可以是 video-only。PR9 的 AAC/ADTS/MPEG-TS 音频封装只能作为历史测试音验证；真实系统播放音频待 PR14 实现和验证。

PR14 证据必须分层记录：

```text
ffprobe 出现 audio=aac：只证明有 AAC 音轨。
logcat 出现 PCM peak / RMS：证明读到非静音 PCM。
PC / Kodi / ffplay 能听到视频 App 声音：才说明真实播放音频链路跑通。
```

不允许只凭 `audio=aac` 写“真实系统音频已达成”。
